...
function getCleanMerchant(label: string): string {
    ...
    if (clean.includes('LCL')) return 'LCL';
    ...
    if (clean.includes('MAGD_SEBASTOPOL') || clean.includes('MAGD SEBASTOPOL')) return 'MAGD SEBASTOPOL';
    if (clean.includes('TABAC_CHATELET') || clean.includes('TABAC CHATELET')) return 'TABAC DU CHATELET';
...
async function updatePennylaneInvoiceLabel(invoiceId: string, label: string) {
    try {
        ...
        if (res.ok) {
            console.log(`   🏷️ Label Pennylane mis à jour : "${label}" (ID: ${invoiceId})`);
            return true;
        } else {
            console.error(`   ⚠️ Échec mise à jour label Pennylane (status ${res.status}) :`, await res.text());
            return false;
        }
    } catch (e: any) {
        console.error(`   ❌ Erreur update label Pennylane :`, e.message);
        return false;
    }
}
...
async function main() {
    ...
    if (!relRes.ok) {
        console.error("❌ Impossible de récupérer le relevé. Assurez-vous que le serveur local est allumé.");
        return;
    }
    ...
    // --- PHASE 1 : TRAITER LES TRANSACTIONS DÉJÀ RAPPROCHÉES ---
    console.log("\n--- 🏁 PHASE 1 : Normalisation des factures déjà rapprochées ---");
    const matchedTxs = proTxs.filter((t: any) => t.matchedInvoice);
    console.log(`🔍 Trouvé ${matchedTxs.length} transactions déjà rapprochées.`);
    ...
    for (const tx of matchedTxs) {
        ...
        const origFilename = decodeURIComponent(tx.matchedInvoice.filename || '');
        ...
        if (fs.existsSync(targetDir)) {
            ...
            // 1. Chercher par nom d'origine exact
            const origMatch = files.find(f => f === origFilename || decodeURIComponent(f) === origFilename);
            if (origMatch) {
                ...
                // 2. Chercher si le fichier propre existe déjà
        ...
        // Si absent localement, on le télécharge depuis Supabase
        if (!fileFound && tx.matchedInvoice.publicFileUrl) {
            console.log(`📥 Absent localement : "${cleanFilename}". Téléchargement depuis Supabase...`);
            try {
        ...
        // Mettre à jour le label sur Pennylane
        if (fileFound) {
            await updatePennylaneInvoiceLabel(tx.matchedInvoice.id, cleanLabel);
        }
    }
    ...
    console.log("\n🎉 Traitement global de synchronisation et de renommage terminé !");
}
    ...
    try {
        ...
        if (!uploadRes.ok) {
            ...
            if (uploadRes.status === 409 || errTxt.includes('already exists with such attachment')) {
                ...
                if (match && match[1]) {
                    invoiceId = match[1];
                    console.log(`ℹ️ La facture existe déjà sur Pennylane avec l'ID ${invoiceId}.`);
                }
            } else {
