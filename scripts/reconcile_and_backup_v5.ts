...
async function main() {
    ...
    for (const tx of matchedTxs) {
        ...
        if (!fileFound && tx.matchedInvoice.publicFileUrl) {
            let fileUrl = tx.matchedInvoice.publicFileUrl;
            if (!fileUrl.startsWith('http')) {
                const cleanPath = fileUrl.replace(/^\/invoices\//, '');
                fileUrl = `https://eqcjgucfpmhvxkckokwb.supabase.co/storage/v1/object/public/invoices/${cleanPath}`;
            }
        ...
        if (fileFound) {
            ...
            if (pennylaneInv) {
                ...
                console.log(`⚠️ Facture non trouvée sur Pennylane pour : "${cleanFilename}"`);
            }
        }
    }
